...
def loso(dataset, pseudo_y, final_images, final_samples, k):
    ...
    for video_index in range(len(final_samples)):
        count_videos += len(final_samples[video_index])
        index = sum(videos_len[:count_videos])
        groups_label[prev_index:index] = video_index
        print('Subject', video_index, ':', prev_index, '->', index)
        prev_index = index

    X = [frame for video in dataset for frame in video]
    print('\nTotal X:', len(X), ', Total y:', len(y))
    return X, y, groups_label
