...
def extract_preprocess(final_images, k):
    global x11, y61, x61, y11
    predictor_model = "Utils\\shape_predictor_68_face_landmarks.dat"
    # 获取脸检测器？
    face_detector = dlib.get_frontal_face_detector()
    face_pose_predictor = dlib.shape_predictor(predictor_model)
    dataset = []
    for video_count in range(len(final_images)):
        off_video = []
        print(final_images[video_count].shape[0])
        for img_count in range(final_images[video_count].shape[0] - k):
            img1 = final_images[video_count][img_count]
            img2 = final_images[video_count][img_count + k]
            if img_count == 0:
                reference_img = img1
                # 只检测一张脸？
                ...
                while len(detect) == 0:
                    print(len(detect))
                    next_img += 1
                    reference_img = final_images[video_count][img_count + next_img]
                    detect = face_detector(reference_img, 1)
